...
data class Key<T : Any>(val name: String, internal val kClass: KClass<T>)
...
open class TypedKeyMap(protected val map: MutableMap<Key<*>, Any> = ConcurrentHashMap()) {
    operator fun <T : Any> get(key: Key<T>): T? = map[key]?.let { key.kClass.cast(it) }
    ...
    fun <T : Any> computeIfAbsent(key: Key<T>, mappingFunction: (Key<T>) -> T): T {
        val value = map.computeIfAbsent(key) { mappingFunction(key) }
        return key.kClass.cast(value)
    }
}
