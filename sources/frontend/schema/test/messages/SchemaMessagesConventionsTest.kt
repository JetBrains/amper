...
class SchemaMessagesConventionsTest {

    @Test
    fun messagesAreSortedAlphabetically() {
        val bundleText = javaClass.getResource("/messages/SchemaBundle.properties")?.readText()
            ?: error("SchemaBundle.properties not found")
        ...
        assertEquals(keys.sorted(), keys, "SchemaBundle.properties should be sorted alphabetically (to avoid git conflicts, and for easier navigability of the file)")
    }
}
