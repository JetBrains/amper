---
description: This page describes how to set up your IDE to work with Kargo.
---
# IDE Setup

??? question "Do I need to use IntelliJ IDEA?"

    Kargo is a command line tool that stands on its own, so using IntelliJ IDEA is not required.
    If you prefer to work directly with the terminal or in another IDE, check out the [Kargo CLI](../cli/index.md).

    However, to make the most out of Kargo and its toolability, we recommend using IntelliJ IDEA.
    There are tons of diagnostics and quick fixes that make your life a bliss when working with Kargo.
...
2. Make sure to install the [:jetbrains-amper: Kargo plugin](https://plugins.jetbrains.com/plugin/23076-amper):
