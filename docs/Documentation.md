...
An Amper **project** is defined by a `project.yaml` file. 
The folder with the `project.yaml` file is the project root. Modules can only be located under the project root.
If there is only one module in the project, a `project.yaml` file is not required.
...
The Amper plugin needs to be added in the `settings.gradle.kts`:
...
plugins {
    // Add the plugin
    id("org.jetbrains.amper.settings.plugin").version("0.3.1-dev-581")
}
```
